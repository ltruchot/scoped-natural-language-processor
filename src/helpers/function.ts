...
export const curry = <T extends any[], R>(fn: (...args: T) => R): VariadicCurry<T, R> => {
  const argN = fn.length;
  const final = (...args:any[]) => {
    if (args.length < argN) {
      return curry((fn as any).bind(final, ...args));
    }
    return fn.call(final, ...args as T);
  };
  return final as VariadicCurry<T, R>;
};
