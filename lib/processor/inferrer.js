...
exports.infer = Either_1.chain(function (_a) {
    ...
    var isFound = ramda_1.pipe(ramda_1.map(ramda_1.prop('key')), ramda_1.map(findConcept), ramda_1.map(ramda_1.prop('is')), ramda_1.any(findPath(words, understood)))(config);
    return isFound
        ? Either_1.right(__assign(__assign({}, inferred), { words: words, config: config, solution: solution, understood: understood }))
        : Either_1.left(__assign(__assign({}, inferred), { config: config, errors: [errors_1.getError(8)] }));
});
